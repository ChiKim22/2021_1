...
//		stackTest2();
	...
	public static void stackTest3() {
		 ...
		 * 만약 empty가 아니면 잘못된 식이다.
		 * ((()) = > 처리후에 empty가 아니게되고,
		 * ()) => pop 했을 때 인출되는 원소가 있어야 하는데,
		 * 없는 경우이며, 이 경우도 잘못된 식이다.
		 * 2 * ( 3 + 2 ) / ( 3 + 1 ) / 2 * 5 - 1 + 10
		 */
		...
		StringTokenizer st = new StringTokenizer(exp); // 이로 인해 여는 괄호에 띄어쓰기를 해줘서 구분 해줘야한다.

		// 3. 토큰을 하나씩 뜯어 보면서 여는 괄호이면 그택에 push, 닫는 괄호이면 스택에서 pop
		Stack<String> stack = new Stack<>();
		while (st.hasMoreTokens()) { // st.hasMoreElements 도 가능
		...
		// 흐름이 여기까지 왔다는 것은,
		// 식에 잘못된 것이 없거나, 여는 괄호가 더 많은 경우이다.
		// 모든 토큰의 처리가 끝나고, 그 때까지 잘못된 괄호가 발견되지 않았다.
		...
		public static void stackTest2(){
			String str = "Apple, Banana, Carro,t Pineapple, Grape, Mango, Strawberry "
					+ "Watermelon, melon, orange, coconut, kiwi, lemon, "
					+ "tomato, cherry, blueberry, peach, cramberry";

			/*
			 * String s = str.substring(0, "apple".length());
			 * System.out.println(s);
			 * 공백문자를 기준으로 하나씩 자르고, 구분자를 입력으로 주고, 그 구분자(delimiter)로 구분되는
			 * 문자열들을 하나씩, 하나씩 뽑아 쓸 수 있게 해주는 java.util 패키지의 클래스는 StringTokennizer
			 */
...
//			System.out.println(s);
			 ...
			 * 토큰의 수만큼 for 문 돌기
			 * 방법 2
			 * 아직 남은 토큰이 있는지 알아보고 있으면 nextToken() 메소드를 호출해서 하나씩 받아서 사용.
			 * 주로 방법 2 를 많이 사용
			 */
